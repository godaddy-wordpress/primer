 ...
 * @since  1.0.0
 */
function the_primer_wc_setup() {

	add_theme_support( 'woocommerce' );

}
add_action( 'after_setup_theme', 'the_primer_wc_setup' );
 ...
 * @since  1.0.0
 */
function the_primer_wc_wrapper_start() {
...
add_action( 'woocommerce_before_main_content', 'the_primer_wc_wrapper_start' );
 ...
 * @since  1.0.0
 */
function the_primer_wc_wrapper_end() {
...
add_action( 'woocommerce_after_main_content', 'the_primer_wc_wrapper_end' );
 ...
 * @since  1.0.0
 */
function the_primer_wc_sidebar() {

	get_sidebar( 'tertiary' );

}
add_action( 'woocommerce_sidebar', 'the_primer_wc_sidebar' );

/**
 * Display WooCommerce messages above post and page content.
 *
 * @action the_primer_before_post_content
 * @action the_primer_before_page_content
 * @since  1.0.0
 */
function the_primer_wc_shop_messages() {
...
add_action( 'the_primer_before_post_content', 'the_primer_wc_shop_messages' );
add_action( 'the_primer_before_page_content', 'the_primer_wc_shop_messages' );
 ...
 * @return string
 */
function the_primer_wc_shop_layout( $layout ) {

	if ( is_shop() ) {

		remove_filter( 'the_primer_current_layout', __FUNCTION__ ); // Prevent infinite loop

		$layout = the_primer_get_layout( wc_get_page_id( 'shop' ) );
...
add_filter( 'the_primer_current_layout', 'the_primer_wc_shop_layout' );

/**
 * Filter the WooCommerce shop page title.
 *
 * @filter the_primer_the_page_title
 * @since  1.0.0
 *
 * @param  string $title
 *
 * @return string
 */
function the_primer_wc_shop_title( $title ) {
...
add_filter( 'the_primer_the_page_title', 'the_primer_wc_shop_title' );
 ...
 * @return int
 */
function the_primer_wc_shop_columns( $columns ) {

	global $post;

	$page_id = ( is_shop() ) ? wc_get_page_id( 'shop' ) : $post->ID;

	if ( 0 === strpos( the_primer_get_layout( absint( $page_id ) ), 'three-column-' ) ) {

		add_filter( 'post_class', 'the_primer_wc_product_classes' );
...
add_filter( 'loop_shop_columns',                    'the_primer_wc_shop_columns' );
add_filter( 'woocommerce_related_products_columns', 'the_primer_wc_shop_columns' );
add_filter( 'woocommerce_upsells_products_columns', 'the_primer_wc_shop_columns' );

/**
 * Filter the WooCommerce product class.
 *
 * @filter the_primer_wc_product_classes
 * @global WP_Post $post
 ...
 * @since 1.0.0
 */
function the_primer_wc_product_classes( $classes ) {
 ...
 * @return array
 */
function the_primer_wc_colors( $colors ) {
	...
	return the_primer_array_replace_recursive( $colors, $wc_colors );

}
add_filter( 'the_primer_colors', 'the_primer_wc_colors' );
